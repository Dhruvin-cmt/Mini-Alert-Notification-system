...
export default function Unread() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const unreadList = ary.filter((msg) => !msg.isRead);
        if (!cancelled) setArr(unreadList);
      } catch (error) {
        if (!cancelled) setArr([]);
      } finally {
  ...
  const handleMarkAllRead = async () => {
    try {
      ...
      setArr([]); // On the unread page, they all disappear
    } catch (error) {
  ...
  return (
    <div className="d-flex flex-column gap-4 p-2">
      <div className="d-flex justify-content-between align-items-center mb-1">
        <h4 className="m-0 fw-bold text-main">Unread Messages</h4>
        <MarkAllRead onClick={handleMarkAllRead} count={arr.length} />
      </div>
      ...
      <div className="message-list-container">
        ...
        {arr.length === 0 && !loading ? (
          <div className="text-center p-5 text-muted border rounded-4 bg-light bg-opacity-50">
            <p className="fs-5 m-0 italic">No unread messages found.</p>
          </div>
        ) : (
          <div
